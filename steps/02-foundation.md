...
When prompted:
- Select the existing Firebase project from step 2
- Set public directory to `dist`
- Configure as single-page app: **Yes**
- Don't overwrite `index.html`

This generates `firebase.json` and `.firebaserc` automatically.
...
Populate this based on the file listing Jack provides from prerequisite step 8. Derive readable titles from filenames (Jack can adjust these later).
...
- For PDFs: renders using `react-pdf`'s `<Document>` and `<Page>` components
  - Configure the PDF.js worker on app init (`pdfjs.GlobalWorkerOptions.workerSrc`)
...
### 13. Verify Firebase Hosting config

`firebase.json` and `.firebaserc` should already exist from prerequisite step 10. Verify that `firebase.json` has the Firestore rules path added:
